...
async def departman_listesi_getir(business_id: str, token: str) -> dict:
    """İşletmeye ait tüm departmanların listesini getirir.
    GET /api/Department/business/{businessId}
    """
    url = f"{BACKEND_API_URL}/api/Department/business/{business_id}"
    try:
        async with httpx.AsyncClient(timeout=_TIMEOUT) as client:
            resp = await client.get(url, headers=_headers(token))
            return _parse_response(resp)
    except Exception as exc:
        logger.exception("departman_listesi_getir hatası")
        return {"hata": str(exc)}
...
async def izin_talebi_olustur_api(
    ...
    def _format_date(d_str: str, is_end: bool = False) -> str:
        try:
            ...
            # C# Backend PostgreSQL veritabanına kaydederken UTC ('Z') formatı zorunlu.
            # Bitiş tarihini 23:59:59 yaparsak, Türkiye saatiyle (UTC+3) ertesi güne (02:59:59) sarkıyor ve sistem 2 gün sayıyor!
            # Bu yüzden her zaman gece 00:00:00 olarak ve Z ekleyerek gönderiyoruz.
